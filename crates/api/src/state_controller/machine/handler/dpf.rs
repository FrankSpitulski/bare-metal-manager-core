...
/// Build a health report that prevents new allocations while DPF provisioning
/// performs disruptive operations on the host.
fn dpf_maintenance_health_report() -> HealthReport {
    HealthReport {
    ...
    match dpf_state {
        ...
        DpfState::WaitingForReady { phase } => {
            ...
            if !is_in_dpf_maintenance(state) {
                ...
                let report = dpf_maintenance_health_report();
                if phase_changed {
            ...
            if current_phase == carbide_dpf::DpuPhase::Error {
                ...
                .in_transaction(&pool, move |txn| {
                    async move {
                        let _ = db::machine::remove_health_report_override(
                            txn,
                            &host_id,
                            OverrideMode::Merge,
