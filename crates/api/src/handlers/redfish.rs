...
pub async fn redfish_apply_action(
    ...
    for (uri, index) in uris {
        ...
        tokio::spawn({
            ...
            let credential_manager = api.credential_manager.clone();
...
async fn handle_request(
    ...
    let (metadata, new_uri, mut headers, http_client) = match (
        create_client(uri, pool, credential_manager, bmc_proxy).await,
...
> {
    ...
    let metadata =
        crate::handlers::bmc_metadata::get_inner(bmc_metadata_request, pool, credential_manager)
            .await?;
