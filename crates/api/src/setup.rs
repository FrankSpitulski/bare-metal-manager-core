...
pub async fn start_api(
    ...
    let dpf_sdk: Option<Arc<dyn crate::dpf::DpfOperations>> = if carbide_config.dpf.enabled {
        ...
        let sdk = Arc::new(
            carbide_dpf::DpfSdk::new(repo, carbide_dpf::NAMESPACE)
                .with_labeler(crate::dpf::CarbideDPFLabeler),
        );

        // Get BMC password for initialization
        ...
        let credentials = vault_client.get_credentials(&key).await?;
        ...
        let services: Vec<carbide_dpf::ServiceDefinition> = carbide_config
            .dpf
            .services
            .as_deref()
            .unwrap_or(&[])
            .iter()
            .map(|s| {
                carbide_dpf::ServiceDefinition::new(
                    &s.name,
                    &s.helm_repo_url,
                    &s.helm_chart,
                    &s.helm_version,
        ...
        let init_config = carbide_dpf::DpfInitConfig {
        ...
        if let Err(err) = sdk.create_initialization_objects(&init_config).await {
            tracing::error!("Failed to create DPF CRDs: {err}");
            return Err(eyre::eyre!("Failed to create DPF CRDs: {err}"));
        }
        Some(Arc::new(crate::dpf::DpfSdkOps::new(sdk, db_pool.clone())))
    } else {
        None
    };
