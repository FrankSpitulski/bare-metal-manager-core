...
///   1. Enters maintenance (health override written)
///   2. Releases maintenance hold, sees reboot required, issues ForceRestart
///   3. After reboot_completed, clears annotation, device ready -> HostInit
#[crate::sqlx_test]
async fn test_waiting_for_ready_reboot_flow(pool: sqlx::PgPool) {
    ...
    let actions = env
        ...
        .all_hosts();
    assert!(
        actions.contains(&RedfishSimAction::Power(SystemPowerControl::ForceRestart)),
        "Expected ForceRestart to be sent, actions: {:?}",
        actions
    );
...
/// WaitingForReady idempotent reboot: ForceRestart is only sent once,
...
async fn test_waiting_for_ready_idempotent_reboot(pool: sqlx::PgPool) {
    ...
    assert_eq!(
        reboot_count, 1,
        "ForceRestart should be sent exactly once (idempotent guard), got {}",
        reboot_count
    );
    ...
    assert_eq!(
        reboot_count2, 0,
        "No additional ForceRestart should be sent while waiting for reboot, got {}",
        reboot_count2
    );
}
