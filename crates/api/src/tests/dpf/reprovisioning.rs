...
/// All DPUs on a multi-DPU host must be reprovisioned in a single state
/// controller iteration. The handler iterates `state.dpu_snapshots` and
/// calls `reprovision_dpu` for each before returning a bulk `Transition`.
#[crate::sqlx_test]
async fn test_multi_dpu_reprovisioning_all_in_single_iteration(pool: sqlx::PgPool) {
    let device_ready = Arc::new(AtomicBool::new(true));
    let registered_devices = Arc::new(Mutex::new(Vec::new()));
    let reprovisioned_devices = Arc::new(Mutex::new(Vec::new()));
    let dpf_sdk: Arc<dyn crate::dpf::DpfOperations> = Arc::new(capturing_mock(
    ...
    assert_eq!(
        reprovisioned, expected,
        "All DPUs must be reprovisioned in a single iteration.\n\
         Reprovisioned: {reprovisioned:?}\n\
         Expected:      {expected:?}"
    );
        ...
        ManagedHostState::DPUReprovision { dpu_states } => {
            for (dpu_id, state) in &dpu_states.states {
                assert!(
                    ...
                    "DPU {dpu_id} should be in WaitingForReady after single-iteration reprovisioning, got: {state:?}"
                );
            }
        }
        other => {
